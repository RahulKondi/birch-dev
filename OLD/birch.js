...
var connect = function (nickname, server, callback) {
  var client;
  if (!connections[nickname] === nickname) {
    ...
    client = connections[nickname][server].client;
  }
  ...
  client.nick(nickname, function (){
    client.on('data', function (message) {
      ...
      if (message.command === 'ERR_NICKNAMEINUSE'){
        client.nick(nickname+"_");
        connections[nickname][server].nick = nickname+"_";
        console.log("Registered nickname :" + connections[nickname][server].nick);
      }
    });
